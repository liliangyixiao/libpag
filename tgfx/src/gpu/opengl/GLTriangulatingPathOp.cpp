...
std::unique_ptr<GLTriangulatingPathOp> GLTriangulatingPathOp::Make(const Path& path,
  ...
  auto mesh = Mesh::MakeFrom(path, &clipBounds);
  if (mesh == nullptr) {
    return nullptr;
  }
  auto pathMesh = static_cast<TriangularPathMesh*>(mesh.get());
  return GLTriangulatingPathOp::Make(std::move(pathMesh->vertices()), pathMesh->vertexCount(),
                                     pathMesh->bounds());
}

std::unique_ptr<GLTriangulatingPathOp> GLTriangulatingPathOp::Make(std::vector<float> vertex,
                                                                   int vertexCount, Rect bounds) {
  return std::unique_ptr<GLTriangulatingPathOp>(
      new GLTriangulatingPathOp(std::move(vertex), vertexCount, bounds));
}
